...
export async function scrapeLiveInstagramProfile(handleInput) {
  ...
  // 1. If Meta Graph API Token is present, attempt Graph API lookup
  if (metaToken && metaToken !== 'your_meta_instagram_access_token_here') {
    try {
      console.log(`[Instagram API] Querying Meta Graph API for business discovery: @${cleanHandle}`);
      ...
      console.error('[Instagram API] Meta Graph API query failed:', err.message);
    }
  }

  // 2. Fallback: Honest estimated profile record (clearly labelled)
  ...
  const estimatedIg = {
    ...
    reach_text: `${formatCountInKAndM(estFollowers)} Followers (Est.)`,
    ...
    bio: `Estimated profile record for @${cleanHandle}. Add META_INSTAGRAM_TOKEN for real Graph API metrics.`
  };
